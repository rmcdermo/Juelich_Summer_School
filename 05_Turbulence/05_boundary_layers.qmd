...
Based on the viscous scales, velocity and distance are scaled as

$$
u^+ \equiv \frac{\langle u \rangle}{u_\tau} \quad \text{and} \quad y^+ \equiv \frac{y}{\delta_\nu}
$$

![](./figs/Boundary-Layer_u_vs_y.png){.nostretch height="360" fig-align="center" fig-alt="Dimensionless mean velocity u+ as a function of wall distance y+, showing the viscous, buffer, logarithmic, and outer regions."}
...
- $\mathrm{Pr} \ll 1$, e.g. liquid metals, thermal diffusivity dominates
- $\mathrm{Pr} \approx \mathcal{O}(1)$, e.g. gases, balance of both processes
- $\mathrm{Pr} \gg 1$, e.g. oils, thermal diffusivity is negligible

## Boundary Layer Thickness

Boundary layer thickness for the velocity $\delta_v(x)$ can be estimated with $\mathrm{Re}_x = u_0\, x / \nu$ by:

$$
\delta_v(x)_{\text{laminar}} \approx 5.0 \cdot x \cdot \mathrm{Re}_x^{-\frac{1}{2}}
$$
$$
\delta_v(x)_{\text{turbulent}} \approx 0.37 \cdot x \cdot \mathrm{Re}_x^{-\frac{1}{5}}
$$ 

Using Pr, the thermal boundary thickness can be related to the velocity in a laminar flow:

$$
\delta_T(x)_{\text{laminar}} = \delta_v(x)_{\text{laminar}} \cdot \mathrm{Pr}_x^{-\frac{1}{3}}
$$
...
$$
h = \frac{k}{L} \max \left( \mathrm{Nu}_{\text{free}}, \mathrm{Nu}_{\text{forced}}\right)
$$

In FDS, the characteristic length is chosen as $L=1\,\mathrm{m}$.
...
$$
\mathrm{Nu}_{\text{free}} =
\left\{
\begin{array}{ll}
\left( 0.825 + 0.324 \, \mathrm{Ra}^{1/6} \right)^2 & \text{Vertical plate or cylinder} \\
0.54 \, \mathrm{Ra}^{1/4} & \text{Horizontal hot plate facing up, } \mathrm{Ra} \leq 10^7 \\
0.15 \, \mathrm{Ra}^{1/3} & \text{Horizontal hot plate facing up, } \mathrm{Ra} > 10^7 \\
0.52 \, \mathrm{Ra}^{1/5} & \text{Horizontal hot plate facing down} \\
\left( 0.60 + 0.321 \, \mathrm{Ra}^{1/6} \right)^2 & \text{Horizontal cylinder} \\
2 + 0.454 \, \mathrm{Ra}^{1/4} & \text{Sphere}
\end{array}
\right.
$$
...
$$
\mathrm{Ra} = \frac{2g \left| T_g - T_w \right| L^3}{(T_g + T_w) \nu \alpha}
$$
...
$$
\mathrm{Nu}_{\text{forced}} = C_0 + \left( C_1 \mathrm{Re}^n - C_2 \right) \mathrm{Pr}^{\frac{1}{3}}
$$
...
$$ 
T^+ = \mathrm{Pr}\, y^+ \quad \text{for}\quad y^+ \leq 11.81
$$

and

$$ 
T^+ = \frac{\mathrm{Pr}_t}{\kappa}\ln(y^+) + B_T(\mathrm{Pr}) \quad \text{with} \quad \text{for}\quad y^+ \leq 11.81
$$
