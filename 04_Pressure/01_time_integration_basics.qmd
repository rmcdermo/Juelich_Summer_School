...
::: {.m-fig-l}
![](https://github.com/rmcdermo/Juelich_Summer_School/releases/download/2026/compartment-oxygen-deflagration.mp4){controls="true" playsinline="true" preload="metadata" poster="Section_1_assets/compartment-oxygen-deflagration-poster.png" fig-alt="Volume rendering of oxygen concentration showing deflagration in a hot scaled compartment, 3408 meshes."}
:::

::: {.m-fig-r}
![](https://github.com/rmcdermo/Juelich_Summer_School/releases/download/2026/ceiling-board-heat-soot.mp4){controls="true" playsinline="true" preload="metadata" poster="Section_1_assets/slide-004-media-01.png" fig-alt="Simulation of heat transfer and soot deposition into a ceiling board, 2000 meshes."}
:::
...
::: {.notes}
- Why the pressure solver matters: meter-scale domains, millimeter cells, thousands of meshes, hundreds of millions of cells, potentially long run times.
- Left: backdraft deflagration, 3408 meshes of 56×64×52 = 635 million cells; 2.5 mm in and just outside the compartment, 5 mm beyond.
- Right: heat transfer and soot deposition into a ceiling board, 2000 meshes of 45×45×30 = 121.5 million cells.
- Every time step of runs like these includes solving the pressure equation across all meshes: today's subject.
:::
...
::: {.notes}
- M = flow speed / speed of sound. In fires ~10/340 m/s, so M ≈ 0.03.
- Sound waves carry nothing we need, so we filter them out: the low Mach approximation.
- Density can still vary strongly with temperature and composition, just not with pressure.
- Payoff: the time step is set by the flow speed, not the sound speed.
:::
...
::: {.notes}
- Pressure = background p̄(z,t) + small hydrodynamic perturbation p̃.
- Only p̄ enters the equation of state: that is what decouples density from the flow's pressure.
- p̄ carries stratification and, in sealed compartments, pressure rise over time (pressure zones).
- Species: the usual advection–diffusion equation with a reaction source.
:::
...
::: {.notes}
- Momentum with the perturbation, or hydrodynamic, pressure p̃ and buoyancy (ρ − ρ₀)g.
- Two vector identities regroup it into F_A, F_B and ∇H.
- The head H = p̃/ρ + |u|²/2, stagnation pressure over density, is the variable the pressure solver works with; we call it pressure too.
:::
...
::: {.notes}
- Compact form: ∂u/∂t = −[F_A + F_B + ∇H].
- F_A: advection (Lamb vector), buoyancy, viscous stress, all known from the current state.
- F_B: baroclinic torque. It contains p̃, which ties it back to H (the pressure iteration, later).
- That leaves H as the one unknown in the momentum equation. H has no evolution (time-derivative) equation; it acts more as a constraint, as we will see.
:::
...
::: {.notes}
- The velocity divergence ∇·u can be factored from the energy equation (sensible enthalpy): heat release rate and transport define the expansion rate.
- Not incompressible: ∇·u ≠ 0 wherever gas is heated or cooled.
- Figure: expansion at the source (> 0), uniform flow (≈ 0), compression (< 0).
- The velocity divergence is known at each step from thermodynamics; the velocity field has to match it.
:::
...
::: {.notes}
- Momentum plus divergence constraint: a saddle-point problem. H is the Lagrange multiplier enforcing the constraint; the solution is a minimum in u and a maximum in H, like the center of a saddle.
- Explicit predictor stage of FDS's 2-stage Runge–Kutta: the F terms at level n are known.
- H is not: it must make u^(n+1) satisfy the constraint.
- Next: fractional step, or projection methods for time integration turn this into a Poisson equation.
:::
...
::: {.notes}
- Take the divergence of the discrete momentum equation and insert the constraint: we end up with a Poisson equation for H.
- Elliptic: H at one point depends on the sources everywhere in the domain.
- Separable operator, which is what makes fast (FFT) solvers possible later.
- The most expensive part of the time step, and it needs boundary conditions for H.
:::
...
::: {.notes}
- Per stage: forces, target divergence, baroclinic term, Poisson solve, projection.
- Steps 1–3 are local and cheap; step 4 is the only global solve.
- Step 5 corrects the intermediate velocity by −Δt∇H so it meets the constraint.
- But there is a catch: F_B needs p̃, and p̃ comes from H.
:::
...
::: {.pi-e3a}
$$\tilde{\boldsymbol{p}}^{k-1}=\rho\left(H^{k-1}-\frac{|\mathbf{u}^{n}|^{2}}{2}\right)$$
:::
...
::: {.notes}
- F_B needs p̃, p̃ needs H: iterate steps 3–4 in k.
- Each pass recovers p̃ from the latest H, updates F_B and solves again.
- Converged, the red terms combine into ∇·[(1/ρ)∇p̃]: the inseparable equation.
- Inseparable accuracy from a separable solver; PRESSURE_TOLERANCE sets how far to iterate (Module II).
:::
...
<!-- The (I,J) marker on the 2D grid: a cell-centre dot with dashed leader lines
     ...
     deck, so they are not in the extracted PNG. The dot is on the centre of
     the cell between the PNG's grid lines x 248.1/285.0 and y 603.5/640.2
     (of 1604x940); the I and J labels sit on its column and row. -->
::: {.sg-marker}
<svg viewBox="0 0 192 126" role="presentation" aria-hidden="true">
<line x1="34" y1="24.61" x2="141.68" y2="24.61" stroke="#1f4e79" stroke-width="2.5" stroke-dasharray="9 7"/>
<line x1="146.28" y1="28.61" x2="146.28" y2="97" stroke="#1f4e79" stroke-width="2.5" stroke-dasharray="9 7"/>
<circle cx="146.28" cy="24.61" r="7" fill="#1f3864"/>
</svg>
:::
...
::: {.notes}
- Cartesian meshes: a cell is (I,J) in 2D, (I,J,K) in 3D.
- Staggered (MAC) layout: H and ρ (scalar fields) at cell centers, velocity (vector field) components on the faces.
- Velocity projection: the face velocity u(I,J) is updated with the H difference across that face (bottom right).
:::
...
::: {.notes}
- Compact Laplacian: ∇·∇H in two steps — gradients on the faces, then their divergence at the center.
- Each face gradient uses only the two cells sharing that face. It is a cell centered finite difference with 2nd order spatial accuracy in uniform grids.
- So H at (I,J) couples only to its four neighbors in 2D in a 5 point stencil. 7 point stencil in 3D.
:::
...
::: {.notes}
- Uniform spacing h gives the 5-point stencil in 2D, 7-point in 3D.
- Applied at each of the N cells, the PDE becomes the linear system A H = R.
- N is the cell count, millions in real runs. Solving this accurately and fast is the rest of the lecture.
:::
...
::: {.notes}
- Discretization Matrix: one row per cell, −4 on the diagonal, 1 for each neighbor, times 1/h².
- Symmetric and very sparse: at most 5 (2D) or 7 (3D) non-zeros per row.
- Sparsity plot: bands at ±1 and ±N_x, come from the cell ordering.
- Boundary conditions decide definiteness: all-Neumann leaves H defined only up to a constant.
:::
...
::: {.notes}
- Boundary conditions enter through ghost cells just outside the mesh.
- Dirichlet: H given on the boundary face, assumed to be the average of the ghost and internal values, H_bc = (H_ghost + H_internal)/2, so ghost = 2·H_bc − H_internal.
- In the stencil the diagonal goes −4 → −5, and 2·H_bc moves to the right-hand side.
- In FDS: OPEN boundaries.
:::
...
::: {.notes}
- Neumann: the normal gradient of H is given, e.g. at solid walls where the normal velocity is known.
- The boundary-face gradient is (H_internal − H_ghost)/Δx, so ghost = H_internal − Δx·∂H/∂x; the diagonal goes −4 → −3 and the gradient term moves to the right-hand side.
- Neumann everywhere makes A singular: H is fixed only up to a constant.
:::
