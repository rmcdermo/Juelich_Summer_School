#!/bin/bash
#
# Run Example 1 and file its output under results/.
#
#   ./run.sh [VARIANT]
#
# VARIANT labels the run: FFT (default), FFT_tight, ULMAT_PARDISO or
# ULMAT_HYPRE.
# Switching solver is the point of the exercise, so it stays a hand edit in
# Example_1.fds -- this script does not touch the &PRES lines. It does check
...
INPUT=Example_1.fds
CHID=Example_1
NP=1          # Example_1.fds defines one &MESH, so one MPI process
...
# the variant. Nothing has to be renamed afterwards, the four runs coexist
...
echo "plot it with:      python eval.py $VARIANT"
