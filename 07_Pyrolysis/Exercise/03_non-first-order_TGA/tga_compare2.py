...
# read in tga data;
tga_file = 'tga_example_data.csv'
tga_data = np.loadtxt(tga_file, delimiter=',', skiprows=1)
T_tga = tga_data[:,1];
M_tga = tga_data[:,2];
MLR_tga = tga_data[:,3];
...
if exists:
        ...
        lfds2=axarr[1].plot(FDS_data[:,1],FDS_data[:,5],'k--',label='FDS')
